            ...
            using (StreamReader reader = new StreamReader(options.InputFile))
            {
                ...
                if (!options.OutputFile)
                {
                    ...
                    if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux) || RuntimeInformation.IsOSPlatform(OSPlatform.FreeBSD))
                    {
                        ...
                        cmds.Add($"{path}/PrecompiledBinaries/unix/tools/avr/bin/avr-gcc -w -Os -g -flto -fuse-linker-plugin -Wl,--gc-sections -mmcu=atmega328p -o {path}/PrecompiledBinaries/unix/tmp/output.ino.elf {path}/PrecompiledBinaries/unix/tmp/sketch/output.ino.cpp.o {path}Core/PrecompiledBinaries/tmp/core_arduino_avr_pro_cpu_16MHzatmega328_db62bc5f977f010e956e85fb47a0c0b7.a -L{path}Core/PrecompiledBinaries/unix/tmp/ -lm");
                        cmds.Add($"{path}/PrecompiledBinaries/unix/tools/avr/bin/avr-objcopy -O ihex -j .eeprom --set-section-flags=.eeprom=alloc,load --no-change-warnings --change-section-lma .eeprom=0 /tmp/output.ino.elf /tmp/output.ino.eep");
                        cmds.Add($"{path}/PrecompiledBinaries/unix/avr-objcopy -O ihex -R .eeprom {path}/PrecompiledBinaries/unix/tmp/output.ino.elf {path}/PrecompiledBinaries/unix/tmp/output.ino.hex");
